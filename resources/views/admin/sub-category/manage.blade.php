...
@section('body')
    <div class="card">
        <div class="card-body">
            ...
            <h4 class="card-title">{{count($sub_categories) > 1?'Show all categories.':(count($sub_categories) == 1?'Show a category':'There is no category.')}}</h4>

            <h5 class="text-center text-success">{{session('message')}}</h5>

            <table id="datatable-buttons" class="table table-dark table-striped table-bordered table-hover table-sm text-center" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                <thead>
                <tr>
                    <th>#</th>
                    <th>Name</th>
                        ...
                        <td class="align-middle">
                            <div class="container">
                                <div class="row justify-content-center">
                                    <a href="{{route('sub-category.edit',['id'=>$sub_category->id])}}" class="btn btn-sm btn-info"><i class="fa fa-edit"></i></a>
                                    <form action="{{route('sub-category.delete',['id'=>$sub_category->id])}}" method="post">
                                        @csrf
